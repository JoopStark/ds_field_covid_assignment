...
global_covid_economic <- big_mac_price_tidy |> 
  left_join(world_gdp_tidy, join_by(name == Country, year)) |> 
  left_join(join_global, join_by(name == Country.Region, year)) |>
  left_join(population_tidy, join_by(name == country)) |> 
  rename(country = name)
...
big_mac_mod <- lm(survival_rate ~ average_big_mac_usd, data=global_covid_economic_transform)
gdp_mod <- lm(survival_rate ~ gdp, data=global_covid_economic_transform)
gdp_per_cap_mod <- lm(survival_rate ~ gdp_per_cap, data= global_covid_economic_transform)
...
global_covid_economic_transform |> 
  ...
  labs(
    ...
    title = "Economy Plays Minimal Role in COVID Survival Based on Countries GDP",
    subtitle = "R² = 0.006",
...
global_covid_economic_transform |> 
  ...
  labs(
    ...
    y = "Survival Rate (Percent)",
    title = "Economy Plays Minimal Role in COVID Survival Based on Countries GDP per Capita",
    subtitle = "R² = 0.056",
...
global_covid_economic_transform |> 
  ...
  labs(
    ...
    y = "Survival Rate (Percent)",
    title = "Economy Plays Minimal Role in COVID Survival Based on informal PPP Analysis",
    subtitle = "R² = 0.014",
